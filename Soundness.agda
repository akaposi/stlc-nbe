...
_≈_ {Γ}{ι}     t t' = t ≡ t'
_≈_ {Γ}{A ⇒ B} t t' = ∀ {Δ}(σ : Ren Δ Γ){a a'} → a ≈ a' → t σ a ≈ t' σ a'
...
⟦Tm⟧ (var v) σ≈δ = ⟦∈⟧ v σ≈δ
⟦Tm⟧ (lam t) σ≈δ ν a≈a' rewrite idrᵣ ν = ⟦Tm⟧ t (≈ₛᵣ ν σ≈δ , a≈a')
⟦Tm⟧ {Δ = Δ} (app f a) σ≈δ = ⟦Tm⟧ f σ≈δ idᵣ (⟦Tm⟧ a σ≈δ)
...
⟦~⟧ {Δ = Δ} (app₁ {x = x} t~t') σ≈δ = ⟦~⟧ t~t' σ≈δ idᵣ (⟦Tm⟧ x σ≈δ)
⟦~⟧ {Δ = Δ} (app₂ {f = f} t~t') σ≈δ = ⟦Tm⟧ f σ≈δ idᵣ (⟦~⟧ t~t' σ≈δ)
...
mutual
  ...
  q≈ {Γ} {ι}     t≈t' = t≈t'
  q≈ {Γ} {A ⇒ B} t≈t' = lam & q≈ (t≈t' (wk {A}) (u≈ {n = var vz} refl))

  u≈ : ∀ {Γ A}{n n' : Ne Γ A} → n ≡ n' → uᴺ n ≈ uᴺ n'
  u≈ {A = ι}     p = ne & p
  u≈ {A = A ⇒ B} p = λ δ a≈a' → u≈ (app & ((_[ δ ]ₙₑᵣ) & p) ⊗ q≈ a≈a')
